/**
 * Test Endpoint: Trigger Worker
 * 
 * GET /api/test/trigger-worker
 * 
 * Manually triggers the worker to process queued jobs.
...
export async function GET(request: NextRequest) {
  ...
  console.log('[Test Worker] Starting job processing...');
  ...
  try {
    ...
    while (Date.now() - startTime < maxRuntime) {
      ...
      if (!job) {
        console.log('[Test Worker] No more pending jobs');
        break;
      }

      const jobStartTime = Date.now();
      console.log(`[Test Worker] Processing job ${job.id} (${job.job_type}), attempt ${job.attempts}`);
    ...
    console.log(`[Test Worker] Complete in ${totalDuration}ms: ${results.length} jobs processed, ${cleanedCount} stuck jobs cleaned`);
...
// Copy all the helper functions from the worker route
async function claimNextJob(supabase: ReturnType<typeof createClient> extends Promise<infer T> ? T : never): Promise<Job | null> {
  const { data: rpcData, error: rpcError } = await supabase
    .rpc('claim_next_job', { p_worker_id: 'test-worker' });
...
async function processJob(supabase: ReturnType<typeof createClient> extends Promise<infer T> ? T : never, job: Job): Promise<JobResult> {
  ...
  try {
    ...
    switch (job.job_type) {
      ...
      case 'select_daily':
        result = await processSelectDaily(job.payload, logger, supabase);
        break;
...
async function processSelectDaily(payload: Record<string, unknown>, logger: InstanceType<typeof PipelineLogger>, supabase: ReturnType<typeof createClient> extends Promise<infer T> ? T : never): Promise<JobResult> {
  try {
...
async function completeJob(
  ...
  console.log(`[Test Worker] Job ${jobId} completed`);
}
...
async function failJob(
  ...
  if (job.attempts >= job.max_attempts) {
    ...
    console.log(`[Test Worker] Job ${job.id} failed permanently after ${job.attempts} attempts: ${error}`);
    ...
    console.log(`[Test Worker] Job ${job.id} scheduled for retry in ${backoffSeconds}s (attempt ${job.attempts}/${job.max_attempts})`);
  }
}
